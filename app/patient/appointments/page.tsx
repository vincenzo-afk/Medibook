...
import { getAppointmentsForPatient, getWaitlistForPatient } from '@/lib/db/queries'
import { Card } from '@/components/ui/Card'
import { AppointmentRow } from '@/components/patient/AppointmentRow'
import { WaitlistSection } from '@/components/patient/WaitlistSection'

export default async function AppointmentsPage() {
  const ctx = await getCtx()
  const [appointments, waitlist] = await Promise.all([
    getAppointmentsForPatient(ctx, 'patient-1'),
    getWaitlistForPatient(ctx, 'patient-1'),
  ])
      ...
      <Card>
        ...
        {upcoming.length === 0 && (
          <p className="px-4 py-6 text-sm text-muted">
            Nothing scheduled.{' '}
            <Link href="/patient/doctors" className="font-medium text-action hover:underline">
              Find a doctor
            </Link>
          </p>
        )}
